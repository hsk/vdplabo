...
class V9918:
    ...
    def render_sprite1(self, surface):
        surface.fill((0, 0, 0))
        for cy in range(self.SCREEN_HEIGHT):
            self.render_line_sprites(surface, cy)
    def render_line_sprites(self, surface, cy):
        sprites_on_line = 0
        for spr in self.sprites:
            ...
            base_size = 16 if self.sprite_size16 else 8
            size = base_size * 2 if self.sprite_mag else base_size
            # TMS9918A: max 4 sprites per line
            if spr["y"] <= cy < spr["y"] + size:
                sprites_on_line += 1
            ...
            for bx, by, pat_no in blocks:
                ...
                for py in range(8):
                    if self.sprite_mag:
                        ...
                        sy = spr["y"] + by + py
                        sh = 1
                    # Skip lines not matching current scanline
                    if not (sy <= cy < sy + sh):
                        continue
                    bits = pattern[py]
                    for px in range(8):
                        ...
                        if self.sprite_mag:
                            ...
                            for ox in range(2):
                                ...
                                if 0 <= sx < self.SCREEN_WIDTH:
                                    surface.set_at((sx, cy), color)
                        else:
                            x = spr["x"] + bx + px
                            if 0 <= x < self.SCREEN_WIDTH:
                                surface.set_at((x, cy), color)
