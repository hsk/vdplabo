"""sc5_sp01.asm と同じ「9個のスプライトのY座標を-8〜8で増減させ、
...
参照:
- ソース: ../asm/sc5_sp01.asm

実行方法:
    python sc5_sp01.py                 # 自動テストのみ実行
    python sc5_sp01.py --show          # pygameウィンドウで目視確認(ループ再生)
    python sc5_sp01.py --update-expected # ../expected/sc5_sp01.webm を再生成
"""
...
# sc5_sp01.asm の sprite_init と同じパラメータ
TEST_COUNT = 9
X = [100 + i * 16 for i in range(TEST_COUNT)]
# sc5_sp01.asm では color_init で d=1..10 の色が各スプライトに設定される (SATの4バイト目の色)
...
def build_vdp() -> V9938:
    ...
    # sc5_sp01.asm は CHGMOD実行直後にWRTVDPでVDPレジスタ7を直接
...
if __name__ == "__main__":
    main(globals(), Simulation, "sc5_sp01", update_expected=update_expected,
