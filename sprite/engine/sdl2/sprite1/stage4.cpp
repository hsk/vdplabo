...
// ----------------------------------------
// stage4.hpp の V9918 クラスを読み込んで動かすだけの薄いドライバ。
// ROM クラスは stage1 と同じ内容(32個のスプライトが8x8/16x16・等倍/拡大を
// 切り替えながら跳ね回るデモ)。レジスタ経由のアクセサを使う点だけが異なる。
#include "stage4.hpp"
...
int main() {
    ...
    while (running) {
        ...
        if (vdp.get_5s()) {
            std::printf("SPRITE OVERFLOW: 5th sprite=%d\n", vdp.get_5s_index());
        }
