    ...
    def render_sprite2(self, surface):
        ...
        for i,spr in enumerate(self.sprites):
            ...
            for bx, by, pat_no in blocks:
                ...
                for py in range(8):
                    bits = pattern[py]
                    color_byte = self.sprite_color_table[i*8+py]
                    cc_bit = True if color_byte & 0x40 else False
                    color_byte &= 0xf
                    color = self.PALETTE[color_byte]
                    for px in range(8):
                        if bits & (0x80 >> px):
                            if self.sprite_mag:
                                x = spr["x"] + (bx + px) * 2
                                ...
                                for oy in range(2):
                                    for ox in range(2):
                                        ...
                                        if 0 <= sx < self.SCREEN_WIDTH and 0 <= sy < self.SCREEN_HEIGHT:
                                            if draw_buffer[sx+self.SCREEN_WIDTH*sy] == 0:
                                                draw_buffer[sx+self.SCREEN_WIDTH*sy] = color_byte
                                                surface.set_at((sx, sy), color)
                                            elif cc_bit:
                                                draw_buffer[sx+self.SCREEN_WIDTH*sy] |= color_byte
                                                surface.set_at((sx, sy), self.PALETTE[draw_buffer[sx+self.SCREEN_WIDTH*sy]])
                            else:
                                x = spr["x"] + bx + px
                                y = spr["y"] + by + py
                                if 0 <= x < self.SCREEN_WIDTH and 0 <= y < self.SCREEN_HEIGHT:
                                    if draw_buffer[x+self.SCREEN_WIDTH*y] == 0:
